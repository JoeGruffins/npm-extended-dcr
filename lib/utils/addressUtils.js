...
var isScriptHash = function isScriptHash(address, coinInfo) {
  if (!isBech32(address)) {
    ...
    var decoded = _utxoLib.address.fromBase58Check(address, coinInfo.network);
  ...
  throw _constants.ERRORS.TypedError('Runtime', 'isScriptHash: Unknown address type');
};
...
var getAddressHash = function getAddressHash(address, coinInfo) {
  ...
  return _utxoLib.address.fromBase58Check(address, coinInfo.network).hash;
};
