...
var GetAddress = /*#__PURE__*/function (_AbstractMethod) {
  ...
  function GetAddress(message) {
    ...
    payload.bundle.forEach(function (batch) {
      ...
      var path = (0, _pathUtils.validatePath)(batch.path, 1);
      var coinInfo;
    ...
    if (_this.params.length === 1) {
      _this.info = (0, _pathUtils.getLabel)('Export #NETWORK address', _this.params[0].coinInfo);
    } else {
  ...
  _proto._call = function _call(_ref) {
    var address_n = _ref.address_n,
        show_display = _ref.show_display,
        multisig = _ref.multisig,
        script_type = _ref.script_type,
        coinInfo = _ref.coinInfo;
    var cmd = this.device.getCommands();
